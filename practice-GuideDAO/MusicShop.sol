...
abstract contract MusicShop is ERC165 {
    enum OrderStatus {
        Paid,
        Delivered
    }
    ...
    function addAlbum(bytes32 uid, string calldata title, uint256 price, uint256 quantity) external onlyOwdner {
        albums.push(
            Album({
                index: currentIndex,
                uid: uid,
    ...
    function delivered(uint256 _index) external onlyOwdner {
